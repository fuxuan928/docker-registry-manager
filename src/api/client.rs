...
use crate::models::{AuthConfig, BlobInfo, CatalogResponse, Manifest, TagsResponse};
...
impl RegistryClient {
    ...
    pub async fn get_tags(&self, repo: &str) -> Result<TagsResponse, ApiError> {
        ...
        if status != 200 {
            return Err(ApiError::from_status(
                status,
                format!("Failed to get tags for {}", repo),
            ));
        }
    ...
    pub async fn get_manifest(
        ...
        repo: &str,
        reference: &str,
    ) -> Result<(Manifest, String), ApiError> {
        ...
        if status != 200 {
            return Err(ApiError::from_status(
                status,
                format!("Failed to get manifest for {}:{}", repo, reference),
            ));
        }
    ...
    pub async fn delete_manifest(&self, repo: &str, digest: &str) -> Result<(), ApiError> {
        ...
        if status != 202 && status != 200 {
            return Err(ApiError::from_status(
                status,
                format!("Failed to delete manifest {}", digest),
            ));
        }
    ...
    pub async fn head_blob(&self, repo: &str, digest: &str) -> Result<BlobInfo, ApiError> {
        ...
        if status != 200 {
            return Err(ApiError::from_status(
                status,
                format!("Failed to get blob {}", digest),
            ));
        }
